...
class SELayer(nn.Module):
    def __init__(self, in_c, out_c, device, kernel_size=1, sigma=1.):
        """
        ...
        if kernel_size is not None:
            ...
            self.gaussin = SpatialGaussianKernel(kernel_size, sigma, out_c, device)
    ...
    def forward(self, R, readout_feats, active_neurons, m_r=None, std_r=None, info_mean=True):
        ...
        # --- 1. Get the mask 'lambda' for one attach feature
        readout = []
...
class SE(nn.Module):
    def __init__(self, in_c, out_c_list, device, kernel_size=1):
        super(SE, self).__init__()
        self.N = len(out_c_list)
        print(f"Initializing SE with in_c: {in_c}, out_c_list: {out_c_list}, N: {self.N}") 
        for i in range(self.N):
            setattr(self, f'iba_{i}', SELayer(in_c, out_c=out_c_list[i], device=device,
                                              kernel_size=kernel_size))
            print(f"Created SELayer: iba_{i}")
