...
class FaceEmbedAlign(TensorDataset):
    ...
    def __getitem__(self, item):
        ...
        Xs = Image.fromarray(Xs)
        with torch.no_grad():
            faces = mtcnn.align_multi(Xs, min_face_size=self.minSize, crop_size=self.crop_size)
        ...
        if random.random() > self.same_prob:
            ...
            while Xt is None:
                ...
                Xt = Image.fromarray(Xt)
                with torch.no_grad():
                    faces = mtcnn.align_multi(Xt, min_face_size=self.minSize, crop_size=self.crop_size)
                if faces is not None:
                    Xt = faces[0]
                else:
                    Xt = None
...
class FaceEmbedAlign256(TensorDataset):
    ...
    def __getitem__(self, item):
        ...
        if random.random() > self.same_prob:
            ...
            same_person = 1
        return self.transforms(Xs), self.transforms(Xt), [
            image_path_s.split('/')[-1].split('.')[0], image_path.split('/')[-1].split('.')[0]]
...
def cut_video(vid_path, start, end, save_dir, name=None):
